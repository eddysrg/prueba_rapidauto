...
<main class="w-11/12 mx-auto">
    ...
    <div class="bg-white p-8 rounded shadow">
        <form action="{{route('lots.update', $lot)}}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            <div class="flex flex-col">
                <label for="name" class="text-sm text-zinc-800 font-semibold">Nombre del local</label>
                <input type="text" id="name" name="name" class="border border-zinc-300 rounded-sm py-1 px-2" value="{{old('name', $lot->name)}}">
                @error('name')
            ...
            <div class="flex flex-col">
                ...
                <input type="text" id="address" name="address" class="border border-zinc-300 rounded-sm py-1 px-2" value="{{old('address', $lot->address)}}">
                @error('address')
                    <span class="text-sm text-red-700 font-semibold mt-1">{{$message}}</span>
                @enderror
            </div>
